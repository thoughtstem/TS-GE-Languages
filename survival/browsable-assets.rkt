...
(define (show-coin-sprites)
  ...
  (define c (inset-frame (draw-entity (gold-coin-entity))))
...
;=== enemy sprite list ====
(define (show-enemy-sprites)

  (define a (inset-frame (draw-entity (slime)))) 
  (define b (inset-frame (draw-entity (cat))))
  (define c (inset-frame (draw-entity (bat))))
  (define d (inset-frame (draw-entity (snake)))) 

  (inset-frame
   (vl-append
    (scale (text "Try these sprites as your enemy!") 1.5)
    (ghost (rectangle 10 10))
    (code+hints a #:settings hints-on-right
                (list a
                      (hint
